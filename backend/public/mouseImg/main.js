...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    mouse.rotation.copy(baseRotation);
...
/* 🎭 외부에서 감정 제어 가능한 함수 */
window.triggerMouseEmotion = function (isHappyEmotion) {
  if (!mouse) {
    console.warn('쥐 모델이 아직 로드되지 않았습니다.');
    return;
  }
...
window.addEventListener('message', (event) => {
  ...
  if (data.mood === 'happy') {
    window.triggerMouseEmotion(true);
  } else if (data.mood === 'angry') {
    window.triggerMouseEmotion(false);
  }
});
...
function animate() {
  ...
  if (mouse) {
    ...
    if (isAngry && t - angryStartTime < ANGRY_DURATION) {
      ...
      // 😡 격렬하게 진동!
      mouse.position.x = Math.sin(t * 50) * 0.5 * intensity;
      mouse.position.y = Math.cos(t * 40) * 0.5 * intensity;

      // 😡 빠르게 회전!
      mouse.rotation.x = baseRotation.x;
      mouse.rotation.y = baseRotation.y + Math.sin(t * 30) * 0.5 * intensity;
      mouse.rotation.z = baseRotation.z + Math.cos(t * 25) * 0.3 * intensity;

      // 😡 크기 변화 (부풀어 오름)
      const angryScale = 1 + intensity * 0.5;
      mouse.scale.set(angryScale, angryScale, angryScale);
      ...
      // 😊 신나게 위아래로 통통!
      mouse.position.x = Math.sin(t * 8) * 0.3 * intensity;
      mouse.position.y = Math.abs(Math.sin(t * 10)) * 0.8 * intensity;

      // 😊 좌우로 흔들흔들
      mouse.rotation.x = baseRotation.x;
      mouse.rotation.y = baseRotation.y + Math.sin(t * 5) * 0.3 * intensity;
      mouse.rotation.z = baseRotation.z + Math.sin(t * 4) * 0.2 * intensity;

      // 😊 크기 변화 (반짝반짝)
      const happyScale = 1 + Math.sin(t * 15) * 0.1 * intensity;
      mouse.scale.set(happyScale, happyScale, happyScale);
      ...
      // 🐭 통통 바운스
      mouse.position.x = 0;
      mouse.position.y = Math.sin(t * 2) * 0.2;

      // 🐭 살짝 좌우 회전
      mouse.rotation.x = baseRotation.x;
      mouse.rotation.y = baseRotation.y + Math.sin(t * 1.5) * 0.15;
      mouse.rotation.z = baseRotation.z;

      // 🐭 숨 쉬는 느낌
      const scale = 1 + Math.sin(t * 3) * 0.05;
      mouse.scale.set(scale, scale, scale);
    }
  }

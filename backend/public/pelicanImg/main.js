...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    // 크기와 위치 조정
    pelican.scale.set(1, 1, 1);
    pelican.position.set(0, 0, 0);

    // 초기 회전값 설정
    baseRotation.set(0, 0, 0); // Pitch 0도, Yaw 0도, Roll 0도
    pelican.rotation.copy(baseRotation);

    // 원래 재질 색상 저장
    pelican.traverse((child) => {
      if (child.isMesh) {
    ...
    console.log('3D 모델 로드 완료!', pelican);
  },
  (progress) => {
    console.log(
      '로딩 중...',
...
/* 🎭 외부에서 감정 제어 가능한 함수 */
window.triggerPelicanEmotion = function (isHappyEmotion) {
  if (!pelican) {
    console.warn('펠리컨 모델이 아직 로드되지 않았습니다.');
    return;
  }
  ...
  if (isHappyEmotion) {
    ...
    console.log('🐦💖 펠리컨이 기분 좋아해요!');
  } else {
    // 😡 빡침 발동!
    isAngry = true;
    isHappy = false;
    angryStartTime = clock.getElapsedTime();
    console.log('🐦💢 펠리컨이 화났어요!');
  }
};
...
function animate() {
  ...
  if (pelican) {
    ...
    if (isAngry && t - angryStartTime < ANGRY_DURATION) {
      ...
      // 😡 격렬하게 진동!
      pelican.position.x = Math.sin(t * 50) * 0.5 * intensity;
      pelican.position.y = Math.cos(t * 40) * 0.5 * intensity;

      // 😡 빠르게 회전!
      pelican.rotation.x = baseRotation.x;
      pelican.rotation.y = baseRotation.y + Math.sin(t * 30) * 0.5 * intensity;
      pelican.rotation.z = baseRotation.z + Math.cos(t * 25) * 0.3 * intensity;

      // 😡 크기 변화 (부풀어 오름)
      const angryScale = 1 + intensity * 0.5;
      pelican.scale.set(angryScale, angryScale, angryScale);
      ...
      // 😊 신나게 위아래로 통통!
      pelican.position.x = Math.sin(t * 8) * 0.3 * intensity;
      pelican.position.y = Math.abs(Math.sin(t * 10)) * 0.8 * intensity;

      // 😊 좌우로 흔들흔들
      pelican.rotation.x = baseRotation.x;
      pelican.rotation.y = baseRotation.y + Math.sin(t * 5) * 0.3 * intensity;
      pelican.rotation.z = baseRotation.z + Math.sin(t * 4) * 0.2 * intensity;

      // 😊 크기 변화 (반짝반짝)
      const happyScale = 1 + Math.sin(t * 15) * 0.1 * intensity;
      pelican.scale.set(happyScale, happyScale, happyScale);
      ...
      // 🐦 통통 바운스
      pelican.position.x = 0;
      pelican.position.y = Math.sin(t * 2) * 0.2;

      // 🐦 살짝 좌우 회전
      pelican.rotation.x = baseRotation.x;
      pelican.rotation.y = baseRotation.y + Math.sin(t * 1.5) * 0.15;
      pelican.rotation.z = baseRotation.z;

      // 🐦 숨 쉬는 느낌
      const scale = 1 + Math.sin(t * 3) * 0.05;
      pelican.scale.set(scale, scale, scale);
    }
  }

...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    penguin.rotation.copy(baseRotation);
    ...
    console.log('3D 모델 로드 완료!', penguin);
  },
  (progress) => {
    console.log(
      '로딩 중...',
...
/* 🎭 외부에서 감정 제어 가능한 함수 */
window.triggerPenguinEmotion = function (isHappyEmotion) {
  if (!penguin) {
    console.warn('펭귄 모델이 아직 로드되지 않았습니다.');
    return;
  }
  ...
  if (isHappyEmotion) {
    ...
    console.log('🐧💖 펭귄이 기분 좋아해요!');
  } else {
    // 😡 빡침 발동!
    isAngry = true;
    isHappy = false;
    angryStartTime = clock.getElapsedTime();
    console.log('🐧💢 펭귄이 화났어요!');
  }
};
...
window.addEventListener('message', (event) => {
  ...
  if (data.mood === 'happy') {
    window.triggerPenguinEmotion(true);
  } else if (data.mood === 'angry') {
    window.triggerPenguinEmotion(false);
  }
});
...
function animate() {
  ...
  if (penguin) {
    ...
    if (isAngry && t - angryStartTime < ANGRY_DURATION) {
      ...
      // 😡 격렬하게 진동!
      penguin.position.x = Math.sin(t * 50) * 0.5 * intensity;
      penguin.position.y = Math.cos(t * 40) * 0.5 * intensity;

      // 😡 빠르게 회전!
      penguin.rotation.x = baseRotation.x;
      penguin.rotation.y = baseRotation.y + Math.sin(t * 30) * 0.5 * intensity;
      penguin.rotation.z = baseRotation.z + Math.cos(t * 25) * 0.3 * intensity;

      // 😡 크기 변화 (부풀어 오름)
      const angryScale = 1 + intensity * 0.5;
      penguin.scale.set(angryScale, angryScale, angryScale);
      ...
      // 😊 신나게 위아래로 통통!
      penguin.position.x = Math.sin(t * 8) * 0.3 * intensity;
      penguin.position.y = Math.abs(Math.sin(t * 10)) * 0.8 * intensity;

      // 😊 좌우로 흔들흔들
      penguin.rotation.x = baseRotation.x;
      penguin.rotation.y = baseRotation.y + Math.sin(t * 5) * 0.3 * intensity;
      penguin.rotation.z = baseRotation.z + Math.sin(t * 4) * 0.2 * intensity;

      // 😊 크기 변화 (반짝반짝)
      const happyScale = 1 + Math.sin(t * 15) * 0.1 * intensity;
      penguin.scale.set(happyScale, happyScale, happyScale);
      ...
      penguin.position.y = Math.sin(t * 2) * 0.2;

      // 🐧 살짝 좌우 회전
      penguin.rotation.x = baseRotation.x;
      penguin.rotation.y = baseRotation.y + Math.sin(t * 1.5) * 0.15;
      penguin.rotation.z = baseRotation.z;

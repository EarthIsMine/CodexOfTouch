...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    dolphin.rotation.copy(baseRotation);
    ...
    console.log('3D 모델 로드 완료!', dolphin);
  },
  (progress) => {
    console.log(
      '로딩 중...',
...
/* 🎭 외부에서 감정 제어 가능한 함수 */
window.triggerDolphinEmotion = function (isHappyEmotion) {
  if (!dolphin) {
    console.warn('돌고래 모델이 아직 로드되지 않았습니다.');
    return;
  }
  ...
  if (isHappyEmotion) {
    ...
    console.log('🐬💖 돌고래가 기분 좋아해요!');
  } else {
    // 😡 빡침 발동!
    isAngry = true;
    isHappy = false;
    angryStartTime = clock.getElapsedTime();
    console.log('🐬💢 돌고래가 화났어요!');
  }
};
...
window.addEventListener('message', (event) => {
  ...
  if (data.mood === 'happy') {
    window.triggerDolphinEmotion(true);
  } else if (data.mood === 'angry') {
    window.triggerDolphinEmotion(false);
  }
});
...
function animate() {
  ...
  if (dolphin) {
    ...
    if (isAngry && t - angryStartTime < ANGRY_DURATION) {
      ...
      // 😡 격렬하게 진동!
      dolphin.position.x = Math.sin(t * 50) * 0.5 * intensity;
      dolphin.position.y = Math.cos(t * 40) * 0.5 * intensity;

      // 😡 빠르게 회전!
      dolphin.rotation.x = baseRotation.x;
      dolphin.rotation.y = baseRotation.y + Math.sin(t * 30) * 0.5 * intensity;
      dolphin.rotation.z = baseRotation.z + Math.cos(t * 25) * 0.3 * intensity;

      // 😡 크기 변화 (부풀어 오름)
      const angryScale = 1 + intensity * 0.5;
      dolphin.scale.set(angryScale, angryScale, angryScale);
      ...
      // 😊 신나게 위아래로 통통!
      dolphin.position.x = Math.sin(t * 8) * 0.3 * intensity;
      dolphin.position.y = Math.abs(Math.sin(t * 10)) * 0.8 * intensity;

      // 😊 좌우로 흔들흔들
      dolphin.rotation.x = baseRotation.x;
      dolphin.rotation.y = baseRotation.y + Math.sin(t * 5) * 0.3 * intensity;
      dolphin.rotation.z = baseRotation.z + Math.sin(t * 4) * 0.2 * intensity;

      // 😊 크기 변화 (반짝반짝)
      const happyScale = 1 + Math.sin(t * 15) * 0.1 * intensity;
      dolphin.scale.set(happyScale, happyScale, happyScale);
      ...
      dolphin.position.y = Math.sin(t * 2) * 0.2;

      // 🐬 살짝 좌우 회전
      dolphin.rotation.x = baseRotation.x;
      dolphin.rotation.y = baseRotation.y + Math.sin(t * 1.5) * 0.15;
      dolphin.rotation.z = baseRotation.z;

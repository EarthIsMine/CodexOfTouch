...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    seal.rotation.copy(baseRotation);
...
/* 🎭 외부에서 감정 제어 가능한 함수 */
window.triggerSealEmotion = function (isHappyEmotion) {
  if (!seal) {
    console.warn('물개 모델이 아직 로드되지 않았습니다.');
    return;
  }
  ...
  if (isHappyEmotion) {
    ...
    console.log('🦭💖 물개가 기분 좋아해요!');
  } else {
    // 😡 빡침 발동!
    isAngry = true;
    isHappy = false;
    angryStartTime = clock.getElapsedTime();
    console.log('🦭💢 물개가 화났어요!');
  }
};
...
window.addEventListener('message', (event) => {
  ...
  if (data.mood === 'happy') {
    window.triggerSealEmotion(true);
  } else if (data.mood === 'angry') {
    window.triggerSealEmotion(false);
  }
});
...
function animate() {
  ...
  if (seal) {
    ...
    if (isAngry && t - angryStartTime < ANGRY_DURATION) {
      ...
      // 😡 격렬하게 진동!
      seal.position.x = Math.sin(t * 50) * 0.5 * intensity;
      seal.position.y = Math.cos(t * 40) * 0.5 * intensity;

      // 😡 빠르게 회전!
      seal.rotation.x = baseRotation.x;
      seal.rotation.y = baseRotation.y + Math.sin(t * 30) * 0.5 * intensity;
      seal.rotation.z = baseRotation.z + Math.cos(t * 25) * 0.3 * intensity;

      // 😡 크기 변화 (부풀어 오름)
      const angryScale = 1 + intensity * 0.5;
      seal.scale.set(angryScale, angryScale, angryScale);
      ...
      // 😊 신나게 위아래로 통통!
      seal.position.x = Math.sin(t * 8) * 0.3 * intensity;
      seal.position.y = Math.abs(Math.sin(t * 10)) * 0.8 * intensity;

      // 😊 좌우로 흔들흔들
      seal.rotation.x = baseRotation.x;
      seal.rotation.y = baseRotation.y + Math.sin(t * 5) * 0.3 * intensity;
      seal.rotation.z = baseRotation.z + Math.sin(t * 4) * 0.2 * intensity;

      // 😊 크기 변화 (반짝반짝)
      const happyScale = 1 + Math.sin(t * 15) * 0.1 * intensity;
      seal.scale.set(happyScale, happyScale, happyScale);
      ...
      // 🦭 통통 바운스
      seal.position.x = 0;
      seal.position.y = Math.sin(t * 2) * 0.2;

      // 🦭 살짝 좌우 회전
      seal.rotation.x = baseRotation.x;
      seal.rotation.y = baseRotation.y + Math.sin(t * 1.5) * 0.15;
      seal.rotation.z = baseRotation.z;

      // 🦭 숨 쉬는 느낌
      const scale = 1 + Math.sin(t * 3) * 0.05;
      seal.scale.set(scale, scale, scale);
    }
  }
